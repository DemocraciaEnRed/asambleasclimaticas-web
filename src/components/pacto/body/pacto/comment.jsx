...
import { faThumbsDown as faSolidThumbsDown, faThumbsUp as faSolidThumbsUp, faXmark } from "@fortawesome/free-solid-svg-icons"
...
export default function Comment({ project, comment, urlComment, answerable, deleteCommentFromList }) {
    ...
    const handleDelete = async () => {
        ...
        addAlert('Si esta seguro de eliminar este comentario haga nuevamente click en el botón', 'danger', false, timeToCancel)
        const timeout = setTimeout(() => { setDeleteTime(false) }, timeToCancel);
    ...
    const canDeleteComment = () => {
        ...
        const isAuthor = user.role === 'author' && user._id === project.author._id;
        ...
        <div className="comment-wrapper pt-3">
            <div className="is-flex is-justify-content-space-between">
                        ...
                        <div className="is-inline highlighted">
                            <div className="is-relative is-inline ">
                                {highlighted === project.version &&
                                    ...
                                    <span className="tag is-light is-absolute">Aporte destacado - Recibido en version {comment.highlightedInVersion}</span>
                                    </>
                                }
                            </div>
                            <div className="is-relative is-inline ">
                ...
                <div className="is-flex">
                        ...
                        {user && (user.role === 'admin' || user.role === 'moderator') &&
                            <div className="highlighted">
                                <div className="is-inline is-clickable is-relative" onClick={handleHighlighted}>
                                    <span className={`tag  ${highlighted === project.version ? 'is-light' : 'is-warning'}`} >{highlighted === project.version ? 'quitar destacado' : 'destacar comentario'}</span>
                                ...
                                {comment.article &&
                                    <div className="is-inline is-clickable is-relative" onClick={handleResolved}>
                                        <span className={`tag   ${resolved === project.version ? 'is-light' : 'is-success'}`} >{resolved === project.version ? 'quitar resuelto' : 'resolver comentario'}</span>
                    ...
                    <div className="highlighted">
                        {canDeleteComment() &&
                            <div className="is-inline is-clickable is-relative  " onClick={handleDelete}>
                                <span className="tag is-danger">Eliminar comentario</span>
