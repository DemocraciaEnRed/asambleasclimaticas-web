...
export default function RestorePasswordForm() {
    ...
    const sendEmail = async (event) => {
        ...
        try {
            const response = await axiosServices.post("/auth/forgot", {
                email,
            });
    ...
    return (
        <div className="content has-text-centered">
            <div className="title mb-6">
                <Image
                    src={`/icon/${RESTORE_PASSWORD_TITLE[success].icon}`}
                    width={0}
                    height={0}
                    sizes="100vw"
                    className="icon-card"
                ...
                <h1 className="mb-0 my-3 is-uppercase is-size-2 is-size-3-touch has-text-weight-light">
                    {RESTORE_PASSWORD_TITLE[success].title}
                </h1>
                <span className="has-text-weight-light is-italic is-size-4">
                    {RESTORE_PASSWORD_TITLE[success].title_pt}
                </span>
            </div>
            <div className="box-content">
                <div>
                    <p className="mb-0 ">
                        {RESTORE_PASSWORD_TITLE[success].description}
                    </p>
                    <span className="has-text-weight-light is-italic is-size-7">
                        {RESTORE_PASSWORD_TITLE[success].description_pt}
                    </span>
                </div>
                {success == "prevSend" && (
